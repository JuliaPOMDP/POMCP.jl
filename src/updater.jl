...
function update{R<:ParticleReinvigorator}(updater::NodeUpdater{R}, b_old::BeliefNode, a, o, b=nothing)
    if !haskey(b_old.children[a].children, o)
        new_collection = handle_unseen_observation(updater.node_belief_updater,
                                                   b_old, a, o)
        new_node = ObsNode(o, 0, new_collection, b_old.children[a], Dict{Any,ActNode}())
...
function update(updater::NodeUpdater, b_old::BeliefNode, a, o, b=nothing)
    if !haskey(b_old.children[a].children, o)
...
create_belief(updater::NodeUpdater) = ObsNode()
