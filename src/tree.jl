...
mutable struct ActNode{A, O, BNodeType <: BeliefNode} <: AbstractActNode
...
mutable struct ObsNode{Belief,A,O} <: BeliefNode{Belief,A,O}
    label::O
...
mutable struct RootNode{RootBelief,A,ANodeType} <: BeliefNode{RootBelief}
    N::Int
